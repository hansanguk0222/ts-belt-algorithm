...
type Game = `Game ${number}: ${string}`;

type RGB = `${number} blue` | `${number} red` | `${number} green`;

const parsedGame = (game: string) =>
	pipe(game, S.remove('Game '), S.split(': '), (item) => ({
		id: O.getWithDefault(Number(A.get(item, 0)), 0),
		turns: pipe(
			A.get(item, 1),
			O.map(
				flow(
					...
					A.map(
						flow(
							A.map(S.split(' ')),
							A.reduce([0, 0, 0], (acc, cur) =>
								match([cur[0], cur[1]])
									.with([P._, 'red'], ([colorScore]) => [
										acc[0] + Number(colorScore),
										acc[1],
										acc[2],
									])
									.with([P._, 'green'], ([colorScore]) => [
										acc[0],
										acc[1] + Number(colorScore),
										acc[2],
									])
									.with([P._, 'blue'], ([colorScore]) => [
										acc[0],
										acc[1],
										acc[2] + Number(colorScore),
									])
									.otherwise(() => acc),
							),
						),
			...
			O.getWithDefault<readonly (readonly number[])[]>([]),
		),
	}));

const part1Solution = (
	parsedGameList: Readonly<
		{
			id: number;
	...
	pipe(
		parsedGameList,
		A.reduce(0, (acc, cur) =>
			A.every(cur.turns, (item) => item[0] <= 12 && item[1] <= 13 && item[2] <= 14)
				? acc + cur.id
				: acc,
		),
	);
		...
		A.map((item) =>
			pipe(
				...
				A.reduce(
					...
					(acc, cur) =>
						[
							N.gt(cur[0], acc[0]) ? cur[0] : acc[0],
							N.gt(cur[1], acc[1]) ? cur[1] : acc[1],
							N.gt(cur[2], acc[2]) ? cur[2] : acc[2],
						] as const,
				),
			),
		),
		A.reduce(0, (acc, cur) => acc + cur[0] * cur[1] * cur[2]),
	);

pipe(input, S.split('\n'), A.map(parsedGame), part1Solution, console.log);
pipe(input, S.split('\n'), A.map(parsedGame), part2Solution, console.log);
