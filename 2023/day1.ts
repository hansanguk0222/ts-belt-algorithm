import { A, N, S, pipe } from '@mobily/ts-belt';
...
const singleDigitAlphabetParser = (strArr: Readonly<string[]>) =>
	pipe(
		strArr,
		A.map(S.replaceAll('one', 'one1one')),
		A.map(S.replaceAll('two', 'two2two')),
		A.map(S.replaceAll('three', 'three3three')),
		A.map(S.replaceAll('four', 'four4four')),
		A.map(S.replaceAll('five', 'five5five')),
		A.map(S.replaceAll('six', 'six6six')),
		A.map(S.replaceAll('seven', 'seven7seven')),
		A.map(S.replaceAll('eight', 'eight8eight')),
		A.map(S.replaceAll('nine', 'nine9nine')),
	);

pipe(
	input2,
	S.split('\n'),
	singleDigitAlphabetParser,
	A.map(S.replaceByRe(/[a-z]/g, '')),
	A.map((item) => Number(`${S.head(item)}${S.last(item)}`)),
