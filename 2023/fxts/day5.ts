...
const FIVE_OF_KIND = 7 as const;
const FOUR_OF_KIND = 6 as const;
const FULL_HOUSE = 5 as const;
const THREE_OF_KIND = 4 as const;
const TWO_PAIR = 3 as const;
const ONE_PAIR = 2 as const;
const HIGH_CARD = 1 as const;
const SOME_THING_WRONG = 0;
...
const makeTuple = (arg: string): [string, number] =>
  pipe(arg.trim(), split(' '), ([head, tail]) => [head, Number(tail)] as [string, number]);

const isFiveOfAKind = (item: number[]) =>
  match([item.length, head(item)])
    .with([1, 5], () => true)
    .otherwise(() => false);

const isFourOfAKind = (item: number[]) =>
  match([item.length, head(item), last(item)])
    .with([2, 4, 1], () => true)
    .with([2, 1, 4], () => true)
    .otherwise(() => false);

const isFullHouse = (item: number[]) =>
  match([item.length, head(item), last(item)])
    .with([2, 3, 2], () => true)
    .with([2, 2, 3], () => true)
    .otherwise(() => false);

const isThreeOfAKind = (item: number[]) =>
  match([item.length, head(item), item[1], last(item)])
    .with([3, 3, 1, 1], () => true)
    .with([3, 1, 3, 1], () => true)
    .with([3, 1, 1, 3], () => true)
    .otherwise(() => false);

const isTwoPair = (item: number[]) =>
  match([item.length, head(item), item[1], last(item)])
    .with([3, 2, 2, 1], () => true)
    .with([3, 2, 1, 2], () => true)
    .with([3, 1, 2, 2], () => true)
...
const isOnePair = (item: number[]) =>
  match([item.length, head(item), item[1], item[2], last(item)])
    .with([4, 2, 1, 1, 1], () => true)
    .with([4, 1, 2, 1, 1], () => true)
    .with([4, 1, 1, 2, 1], () => true)
    .with([4, 1, 1, 1, 2], () => true)
...
const isHighCard = (item: number[]) =>
  match([item.length, head(item), item[1], item[2], item[3], last(item)])
